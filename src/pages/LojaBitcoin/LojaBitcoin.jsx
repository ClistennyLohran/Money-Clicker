...
export default function LojaBitcoin() {
  ...
  const { miningPower, setMiningPower, energyPower, setEnergyPower, energyPowerUsed, setEnergyPowerUsed, temperature, setTemperature, temperatureDecrease, setTemperatureDecrease, miningPowerDecrease } = useContext(AdvancedMiningContext);
  ...
  return (
    <motion.div className={styles.container} animate={{ opacity: [0, 1], x: [-600, 0] }}>
      ...
      <div className={styles.displayInfoContainer}>
        <DisplayInfo dataTip="Energia gerada e energia utilizada<br>respectivamente pelas placas de vídeo." twoValues={true} middle={false} icon={<AiFillThunderbolt/>} title="ENERGIA GERADA / UTILIZADA" value={(energyPower * energyPowerBoost) * energyPowerMultiply} value2={energyPowerUsed * energyEconomy} formatter={1} />
        <DisplayInfo dataTip="O poder de mineração de todas as suas<br>placas de vídeo somadas." twoValues={false} middle={true} icon={<GiMiner/>} title="PODER DE MINERAÇÃO" value={((miningPower * miningPowerBoost) * miningPowerMultiply) * miningPowerDecrease} formatter={2} />
        <DisplayInfo dataTip="A temperatura do ambiente<br><br>Se a temperatura estiver entre 50C° e 90C° o poder de mineração será reduzido em 50%.<br>Se a temperatura for maior do que 90C° todas as placas de vídeo serão desligadas.<br>Se a temperatura estiver abaixo de -20C° as placas de vídeo serão desligadas." twoValues={false} middle={false} icon={<FaTemperatureHigh/>} title="TEMPERATURA DO AMBIENTE" value={(temperature + 26) - temperatureDecrease} formatter={3} />
      </div>
